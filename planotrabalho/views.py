...
class PlanoTrabalho(DetailView):
    ...
    def get_context_data(self, **kwargs):
        try:
            ...
            context["data_final"] = data_final.replace(
                year=data_final.year + prazo, day=data_final.day + 1
            )
        except:
            return context
        return context
...
class ListarConselheiros(ListView):
    ...
    def get_queryset(self):
        ...
        conselheiros = Conselheiro.objects.filter(
            conselho=q, situacao=1
        )  # 1 = Habilitado
...
def get_conselheiros(request):
    if request.is_ajax() and request.GET.get("id", None):
        ...
        response = {}
        response["conselheiros"] = list(
            conselheiros.values_list("nome", "email", "segmento")
        )
